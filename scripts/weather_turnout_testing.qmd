...
# Display table with kable formatting
contingency_table_pres %>%
  addmargins() %>%
  kable(caption = "Contingency Table: Weather Conditions × Turnout Level (Presidential Elections)",
...
chi_pres$expected %>%
  kable(caption = "Expected Frequencies (Presidential Elections)",
        digits = 2,
  ...
  row_spec(0, bold = TRUE) %>%
  column_spec(1, bold = TRUE) %>%
  footnote(general = "All expected frequencies ≥ 5. Chi-square assumption satisfied.")
```
...
contingency_table %>%
  addmargins() %>%
  kable(caption = "Contingency Table: Weather Conditions × Turnout Level (Midterm Elections)",
        col.names = c("High", "Low", "Total"),
        align = c("r", "r", "r"),
        booktabs = TRUE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
...
# Check expected frequencies
chi$expected %>%
  kable(caption = "Expected Frequencies (Midterm Elections)",
        digits = 2,
        col.names = c("High", "Low"),
        align = c("r", "r"),
        booktabs = TRUE) %>%
...
There is NO statistically significant association between weather conditions and voter turnout classification in Midterm elections.
...
cat("Weather conditions (dry vs. wet) show NO statistically significant association\n")
cat("with turnout in either election type. However, temperature shows a statistically\n")
cat("significant positive correlation with Midterm turnout (r = 0.328, p = 0.002),\n")
cat("but no correlation with Presidential turnout.\n")
```
